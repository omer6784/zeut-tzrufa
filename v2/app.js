...
function initTitleScene3D() {
  ...
  scene.add(key);
  const fill = new THREE.DirectionalLight(0xfcf7f1, 3);   // beige fill light
  fill.position.set(-4, -3, 1);
  scene.add(fill);
  const rim = new THREE.DirectionalLight(0xf1eb11, 1.5);  // solid yellow rim from back
      ...
      mat.onBeforeCompile = (shader) => {
        ...
        shader.fragmentShader = shader.fragmentShader.replace(
          ...
          `
            ...
            // 3-color procedural gradient using the exact yellow theme: #e8e8d7, #ede99f, #f1eb11
            vec3 c0 = pow(vec3(232.0 / 255.0, 232.0 / 255.0, 215.0 / 255.0), vec3(2.2)); // #e8e8d7
            vec3 c1 = pow(vec3(237.0 / 255.0, 233.0 / 255.0, 159.0 / 255.0), vec3(2.2)); // #ede99f
            vec3 c2 = pow(vec3(241.0 / 255.0, 235.0 / 255.0, 17.0 / 255.0), vec3(2.2));  // #f1eb11

            if (t < 0.5) {
                frontColor = mix(c0, c1, t / 0.5);
            } else {
                frontColor = mix(c1, c2, (t - 0.5) / 0.5);
            }
...
function initHamsaScene3D() {
  ...
  fill.position.set(-4, -3, 1);
  scene.add(fill);
  const rim = new THREE.DirectionalLight(0xf1eb11, 1.5);  // solid yellow rim
  rim.position.set(3, -2, -4);
  scene.add(rim);
  const top = new THREE.DirectionalLight(0xffffff, 1.2);
  top.position.set(0, 8, 2);
