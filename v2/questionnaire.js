...
let BLUE   = '#1a1a1a';
const ORANGE = '#f1eb11';
let BLUE_RGB   = '26,26,26';
const ORANGE_RGB = '241,235,17';
const BG     = '#fcf7f1';
...
const QUESTIONS = [
  ...
  { id:'roots',     label:'שורשים',     text:'עקוב אחר שורש,\nוגלה את הסמל שלך', type:'roots-tree' },
...
function _renderQuestionImpl(idx){
  ...
  if(q.type==='stars'){
    ...
    const items = shuffled.map((w, i) => {
      ...
      const fontFamilies = ["'Shofar', sans-serif", "'ArbelG', sans-serif", "Georgia, serif", "sans-serif"];
      const fontStyles = ["normal", "italic"];
      const initialFamily = fontFamilies[(i * 3) % fontFamilies.length];
...
function updateLoadBar(progress){
  const fill = document.querySelector('.q-header-loadbar-fill');
  if(!fill) return;
  let v;
  if (progress != null) {
    v = progress;
  } else {
    // Stage 1 (design questions). st.current is 0 to 5.
    // Map st.current = 0 to 0.08 (almost empty but visible)
    // Map st.current = 5 to 0.80
    v = 0.08 + (st.current / (QUESTIONS.length - 1)) * 0.72;
  }
  fill.style.setProperty('--q-load-progress', v.toFixed(4));

  const row = document.querySelector('.q-header-steps-row');
  if (row) {
    row.setAttribute('data-covered-1', v >= 0.12 ? 'true' : 'false');
  }
}
