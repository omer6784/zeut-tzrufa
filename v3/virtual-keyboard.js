...
// Outside clicks close the keyboard (use capture to beat input click handlers)
document.addEventListener('mousedown', (e) => {
  if (!kbEl || kbEl.classList.contains('vk-hidden')) return;
  if (kbEl.contains(e.target)) return;
  if (boundInput && (e.target === boundInput || boundInput.contains(e.target))) return;
  hide();
  boundInput = null;
}, true);
