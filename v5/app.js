...
function initTouchSound() {
  ...
  const play = () => {
    ...
    try {
      ...
      // A short HIGH glass-ping, clearly audible but gentle. Tuned to 1296Hz =
      // 432×3 (with a whisper of its octave at 2592 = 432×6), i.e. pure harmonics
      // of the interface's 432Hz ambient track — so the tap always lands IN TUNE
      // with the music, high and bright without ever clashing. A tiny settling
      // glide keeps it organic ("plick", not beep), and the fast decay keeps it
      // delicate.
      const g = ctx.createGain();
      g.gain.setValueAtTime(0.0001, t);
      g.gain.exponentialRampToValueAtTime(0.09, t + 0.005);   // clear, quick bloom
      g.gain.exponentialRampToValueAtTime(0.0001, t + 0.17);  // short, airy tail
      g.connect(ctx.destination);
      [[1296, 1], [2592, 0.14]].forEach(([f, level]) => {
        const o = ctx.createOscillator();
        o.type = 'sine';
        o.frequency.setValueAtTime(f * 1.02, t);              // starts a hair high…
        o.frequency.exponentialRampToValueAtTime(f, t + 0.04);// …settles: droplet feel
