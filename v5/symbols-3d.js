...
export const SYMBOLS_3D = {
  ...
  cowrie: {
    ...
    color: 0xf5f5ed, dotSize: (size) => Math.max(1.4, size * 0.02),
    animate: (obj, tMs) => { obj.rotation.y = Math.sin(tMs * 0.0011) * 0.3; },  // small right-left turn (shows its depth)
  },
  snake: {
  ...
  /* ── the four user-supplied OBJs (Jul 2026) — SHELL-sampled like the rest. ── */
  algiz: {
    obj: '/algiz.obj', sampler: 'shell', sample: { GRID_SPACING: 4 },
    color: 0xf5f5ed, dotSize: (size) => Math.max(1.4, size * 0.02),
    animate: (obj, tMs) => { obj.rotation.y = Math.sin(tMs * 0.0012) * 0.6; },  // swayY
  },
  triskele: {
    obj: '/triskele.obj', sampler: 'shell', sample: { GRID_SPACING: 4 },
    color: 0xf5f5ed, dotSize: (size) => Math.max(1.4, size * 0.02),
    animate: (obj, tMs) => { obj.rotation.y = tMs * 0.0006; },                  // continuous spin
  },
  solarcross: {
    obj: '/solarcross.obj', sampler: 'shell', sample: { GRID_SPACING: 4 },
    color: 0xf5f5ed, dotSize: (size) => Math.max(1.4, size * 0.02),
    animate: (obj, tMs) => { obj.rotation.y = tMs * 0.0006; },                  // continuous spin
  },
  endlessknot: {
