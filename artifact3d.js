...
function makeMaterial(yMin,yMax,xMin,xMax){
  return new THREE.ShaderMaterial({
    transparent:false,
    uniforms:{
      colorBlue:   { value: new THREE.Color('#0000c8') }, // electric blue
      colorPink:   { value: new THREE.Color('#F73B95') }, // hot pink
      colorOrange: { value: new THREE.Color('#FD7041') }, // coral orange
      yMin: { value: yMin },
      yMax: { value: yMax },
      xMin: { value: xMin },
      xMax: { value: xMax },
...
export function mountArtifact3D(container, data){
  ...
  _renderer.setClearColor(0xfcf7f1, 1);
  ...
  Promise.all([_anahReady, _hamsaReady]).then(()=>{
